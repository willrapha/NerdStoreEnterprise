    ...
    public class AuthController : MainController 
    {
        ...
        public async Task<ActionResult> Registrar(UsuarioRegistro usuarioRegistro)
        {
            ...
            if(result.Succeeded)
            {
                ...
                if (!clienteResult.ValidationResult.IsValid) // Se teve algum erro de negocio na criação do usuario
                {
        ...
        private async Task<ClaimsIdentity> ObterClaimsUsuario(IList<Claim> claims, IdentityUser user)
        {
            ...
            claims.Add(new Claim(JwtRegisteredClaimNames.Nbf, ToUnixEpochDate(DateTime.UtcNow).ToString())); // Nbf - não é possivel apresentar o token antes dessa data
            claims.Add(new Claim(JwtRegisteredClaimNames.Iat, ToUnixEpochDate(DateTime.UtcNow).ToString(), ClaimValueTypes.Integer64)); // Iat - quando o token foi emitido
        ...
        private async Task<ResponseMessage> RegistrarCliente(UsuarioRegistro usuarioRegistro)
        {
            ...
            {
                await _userManager.DeleteAsync(usuario); // no caso de retornar alguma Exception do RabbitMQ ou EasyNetQ
                throw;
            }
        }
    }
